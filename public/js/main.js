...
let Cart = {
    ...
    LOAD: () => {
        let cartQuantity
        if (Object.keys(sessionStorage).includes('cart')) {
            cartQuantity = JSON.parse(sessionStorage.getItem('cart')).items.map(e => e.count).reduce((a, b) => a + b)
        } else {
            cartQuantity = 0
        }
    ...
    VIEW: () => {
        if (Object.keys(sessionStorage).includes('cart')) {
            ...
            for (let i = 0; i < data.items.length; i++) {
                ...
                tdAmount.innerText = `$${data.items[i].price}`
    ...
    DELETE: (e) => {
        ...
        console.log(value.items.map(e => e.name == data.name))
    },
    REMOVE: (e) => {
        let data = JSON.parse(e.target.getAttribute('data-value'))
    }
